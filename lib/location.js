...
function Location (options) {
  ...
  // this.failedSessionCount = 0 TODO: use this
  this.trackingItems = []
...
Location.prototype.getAddressForItem = function (itemId, callback) {
  ...
  checkActiveSession(self, function () {
    getPosition(itemId, self.activeSessionId, function (error, position) {
      if (error) return self.emit('error', {function: 'getAddressForItem > getPosition', error: error.stack})
      getAddress(position, self.userId, function (error, address) {
        if (error) return self.emit('error', {function: 'getAddressForItem > getAddress', error: error.stack})
  ...
  checkActiveSession(self, function () {
    self.trackingItems.forEach(function (item) {
      getPosition(item.itemId, self.activeSessionId, function (error, position) {
        if (error) {
          self.emit('error', {function: 'internal > Tracking > getPosition', error: error, stack: error.stack})
        } else {
        // check diff
          if (position.x !== item.x ||
            ...
            if (item.distance > 0 || !item.address) {
              getAddress(position, self.userId, function (error, body) {
                if (error) return self.emit('error', {function: 'internal > Tracking > getAddress', error: error, stack: error.stack})
                item.address = body
...
function checkActiveSession (obj, callback) {
  if (obj.activeSessionId == null ||
    ...
    login(obj.user, obj.password, function (error, sessionId, userId) {
      if (error) return obj.emit('error', {function: 'internal > checkActiveSession > login', error: error.stack})
      obj.userId = userId
